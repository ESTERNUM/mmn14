...
#include <string.h>
#include <stdlib.h>
//#include "writefiles.h" *** missing file
#include "utils.h"
//#include "first_pass.h" **** missing file
...
static bool process_file(char *filename) {
	...
	long data_imge[CODE_ARR_IMG_LENGTH]; /* Contains an image of the machine code */
	...
	for (curr_line_info.line_number = 1;
		...
		if (strchr(temp_line, '\n') == NULL && !feof(file_des)) {
			...
			if (!process_line_fpass(curr_line_info, &ic, &dc, code_img, data_imge, &symbol_table)) {
				if (is_success) {
	...
	for (curr_line_info.line_number = 1; !feof(file_des); curr_line_info.line_number++) {
		...
		if (is_success) {
			...
			is_success = write_output_files(code_img, data_imge, icf, dcf, filename, symbol_table);
		}
	}
...
/***********************************************
missing closing the files (fclose) (-3)
************************************************/
